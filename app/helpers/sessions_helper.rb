module SessionsHelper
  ...
  def current_user # returns the current logged in user if the page is being accessed by a registered user.
    @current_user ||= User.find_by(id: sessions[:user_id]) #note that the instance variable @current_user is conditionally assigned. More info on that and on the data assigned below.
